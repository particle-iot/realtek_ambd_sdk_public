...
#define RTP_DBG_MODE_INFO 0
#define RTP_DBG_MODE_WARN 1
#define RTP_DBG_MODE_ERR 2

extern int ConfigDebugRTPErr;
extern int ConfigDebugRTPWarn;
extern int ConfigDebugRTPInfo;

#define RTP_DBG_ERROR(...)													\
			do {															\
				if (ConfigDebugRTPErr)										\
					printf("\n\r" RTP_ERR_PREFIX __VA_ARGS__);				\
			}while(0)

#define RTP_DBG_WARNING(...)												\
			do {															\
				if (ConfigDebugRTPWarn)										\
					printf("\n\r" RTP_WARN_PREFIX __VA_ARGS__);				\
			}while(0)

#define RTP_DBG_INFO(...)													\
			do {															\
				if (ConfigDebugRTPInfo)										\
...
void rtp_set_debug_mode(int debug_mode, int enable);
