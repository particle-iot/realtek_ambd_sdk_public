...
static int _os_body_expand(ogg_stream_state *os,long needed){
  if(os->body_storage-needed<=os->body_fill){
    ...
    ret=(void*)_ogg_realloc(os->body_data,body_storage*sizeof(*os->body_data));
    if(!ret){
      ogg_stream_clear(os);
      return -1;
    }
...
static int _os_lacing_expand(ogg_stream_state *os,long needed){
  if(os->lacing_storage-needed<=os->lacing_fill){
    ...
    ret=(void*)_ogg_realloc(os->lacing_vals,lacing_storage*sizeof(*os->lacing_vals));
    if(!ret){
      ogg_stream_clear(os);
      return -1;
    }
    os->lacing_vals=ret;
    ret=(void*)_ogg_realloc(os->granule_vals,lacing_storage*
...
char *ogg_sync_buffer(ogg_sync_state *oy, long size){
  ...
  if(size>oy->storage-oy->fill){
    ...
    //printf("newsize is %d\n", (int)newsize);
    if(oy->data){
      ret=(void*)_ogg_realloc(oy->data,newsize);
	//printf("in _ogg_realloc 1\n");
	}
    else{
      ret=(void*)_ogg_malloc(newsize);
	//  printf("in _ogg_realloc 2\n");
    	}
    if(!ret){
      ogg_sync_clear(oy);
      return NULL;
...
//printf("bodybytes + headbytes are %d\n", (oy->bodybytes+ oy->headerbytes));
...
int ogg_stream_pagein(ogg_stream_state *os, ogg_page *og){
  ...
  if(bodysize){
	...
	//printf("in if bodysize the value of it %d\n", os->body_fill);
    os->body_fill+=bodysize;
	//printf("in if bodysize the value of it after %d\n", os->body_fill);
  }

  {
    int saved=-1;
	//printf("value of segptr is %d\n", segptr);
	//printf("value of segments is %d\n", segments);
    while(segptr<segments){
      int val=header[27+segptr];
    //  printf("lacing fill fill %d\n", os->lacing_fill);
	//  printf("val is %d\n", val);
      os->lacing_vals[os->lacing_fill]=val;
      os->granule_vals[os->lacing_fill]=-1;
	//printf("1 os->lacing_vals is %d\n", os->lacing_vals[os->lacing_fill]);
      if(bos){
        os->lacing_vals[os->lacing_fill]|=0x100;
        bos=0;
      }
	//printf("saved before %d\n", saved);
      if(val<255)saved=os->lacing_fill;
    // printf("saved after %d\n", saved);
	// printf("lacing_fill before %d\n", os->lacing_fill);
      os->lacing_fill++;
	// printf("lacing_fill after %d\n", os->lacing_fill);
      segptr++;
	//printf("before lacing packet is %d\n", os->lacing_packet);
      if(val<255)os->lacing_packet=os->lacing_fill;
	//  printf("after lacing packet is %d\n", os->lacing_packet);
    }
...
//  printf("1 in packetout lacing_packet %d\n", os->lacing_packet);
//printf("1 in packetout lacing_returned %d\n", (int)(os->lacing_returned));
//printf("its value is %d\n", (int)(os->lacing_vals[ptr]));
  ...
  /* Gather the whole packet. We'll have no holes or a partial packet */
  {
    ...
    if(op){
      ...
      op->bytes=bytes;
	//  printf("packet out op bytes are %d\n", bytes);
    }
