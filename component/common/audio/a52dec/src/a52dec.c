...
static int disable_accel = 1;
static int disable_dynrng = 1;
static int disable_adjust = 1;
...
#define FILE_NUM				6
file_info audio_files[FILE_NUM] = {
	{44100, WL_16, 32000, "sin441.ac3"},
	{44100, WL_16, 32000, "noise441.ac3"},
	{32000, WL_16, 32000, "sin32.ac3"},
	{32000, WL_16, 32000, "noise32.ac3"},
	{48000, WL_16, 32000, "sin48.ac3"},
	{48000, WL_16, 32000, "noise48.ac3"},
};
...
static void es_loop (void)
{
    ...
    if(drv_num < 0){
        ...
        return ;
            ...
            logical_drv[1] = drv_num + ':';			
            logical_drv[2] = drv_num + '/';	
            logical_drv[3] = drv_num + 0;	
	}
    ...
    sprintf(&abs_path[strlen(abs_path)], "%s", audio_files[file_index].wav);

    if(f_open(&AC3file, abs_path, FA_READ) != FR_OK){
        printf("Can't open: %s\r\n", abs_path);
        goto umount;
	}
    AC3_file_size = (u32)f_size(&AC3file);
    do {
	//size = fread (buffer, 1, BUFFER_SIZE, in_file);
	/*
	size = (input_bin_size >= BUFFER_SIZE*(cnt+1))?BUFFER_SIZE:(input_bin_size-BUFFER_SIZE*cnt);
        memcpy(buffer, input_bin+BUFFER_SIZE*cnt, size);
        cnt++;
        */
       size = (AC3_file_size >= BUFFER_SIZE*(cnt+1))?BUFFER_SIZE:(AC3_file_size-BUFFER_SIZE*cnt);
	f_read(&AC3file, buffer, size, &bytesLeft);
	time1 = RTIM_GetCount(TIM0);
	a52_decode_data (buffer, buffer + size);
	time2 = RTIM_GetCount(TIM0);
	total_time += (time2-time1)*32;
	cnt++;	
...
int a52dec_main (int argc, char ** argv)
{
    uint32_t accel;
    ao_driver_t * drivers;
