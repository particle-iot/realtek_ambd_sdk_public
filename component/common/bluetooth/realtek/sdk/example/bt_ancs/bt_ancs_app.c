...
/** @addtogroup  PERIPH_SEVER_CALLBACK Profile Server Callback Event Handler
    ...
    * @{
    */
T_SERVER_ID bt_ancs_simp_srv_id; /**< Simple ble service id*/
T_SERVER_ID bt_ancs_bas_srv_id;  /**< Battery service id */
/** @} */ /* End of group PERIPH_SEVER_CALLBACK */
/** @defgroup  PERIPH_GAP_MSG GAP Message Handler
    * @brief Handle GAP Message
    * @{
    */
T_GAP_DEV_STATE bt_ancs_gap_dev_state = {0, 0, 0, 0, 0};                 /**< GAP device state */
T_GAP_CONN_STATE bt_ancs_gap_conn_state = GAP_CONN_STATE_DISCONNECTED; /**< GAP connection state */

/*============================================================================*
 *                              Functions
 *============================================================================*/
void bt_ancs_app_handle_gap_msg(T_IO_MSG  *p_gap_msg);
/**
...
void bt_ancs_app_handle_io_msg(T_IO_MSG io_msg)
{
    ...
    case IO_MSG_TYPE_BT_STATUS:
        {
            bt_ancs_app_handle_gap_msg(&io_msg);
        }
        break;
#if F_BT_ANCS_CLIENT_SUPPORT
    case IO_MSG_TYPE_ANCS:
        {
            bt_ancs_handle_msg(&io_msg);
        }
        break;
#endif
    default:
 ...
 * @return   void
 */
void bt_ancs_app_handle_dev_state_evt(T_GAP_DEV_STATE new_state, uint16_t cause)
{
    APP_PRINT_INFO3("bt_ancs_app_handle_dev_state_evt: init state %d, adv state %d, cause 0x%x",
                    new_state.gap_init_state, new_state.gap_adv_state, cause);
    if (bt_ancs_gap_dev_state.gap_init_state != new_state.gap_init_state)
    {
        if (new_state.gap_init_state == GAP_INIT_STATE_STACK_READY)
        {
            APP_PRINT_INFO0("GAP stack ready");
            printf("[BT ANCS] GAP stack ready\r\n");
            uint8_t bt_addr[6]={0};
            /*stack ready*/
    ...
    if (bt_ancs_gap_dev_state.gap_adv_state != new_state.gap_adv_state)
    {
    ...
    bt_ancs_gap_dev_state = new_state;
}
 ...
 * @return   void
 */
void bt_ancs_app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{
    APP_PRINT_INFO4("bt_ancs_app_handle_conn_state_evt: conn_id %d old_state %d new_state %d, disc_cause 0x%x",
                    conn_id, bt_ancs_gap_conn_state, new_state, disc_cause);
    switch (new_state)
    {
    case GAP_CONN_STATE_DISCONNECTED:
        {
            ...
            {
                APP_PRINT_ERROR1("bt_ancs_app_handle_conn_state_evt: connection lost cause 0x%x", disc_cause);
            }
            printf("[BT ANCS] BT Disconnected, cause 0x%x, start ADV\r\n", disc_cause);
            le_adv_start();
        }
        break;
    ...
    bt_ancs_gap_conn_state = new_state;
}
 ...
 * @return   void
 */
void bt_ancs_app_handle_authen_state_evt(uint8_t conn_id, uint8_t new_state, uint16_t cause)
{
    APP_PRINT_INFO2("bt_ancs_app_handle_authen_state_evt:conn_id %d, cause 0x%x", conn_id, cause);

    switch (new_state)
    {
    case GAP_AUTHEN_STATE_STARTED:
        {
            APP_PRINT_INFO0("bt_ancs_app_handle_authen_state_evt: GAP_AUTHEN_STATE_STARTED");
        }
        break;
...
#endif
                APP_PRINT_INFO0("bt_ancs_app_handle_authen_state_evt: GAP_AUTHEN_STATE_COMPLETE pair success");

            }
            else
            {
                printf("Pair failed: cause 0x%x\r\n", cause);
                APP_PRINT_INFO0("bt_ancs_app_handle_authen_state_evt: GAP_AUTHEN_STATE_COMPLETE pair failed");
            }
        }
        break;
    ...
    default:
        {
            APP_PRINT_ERROR1("bt_ancs_app_handle_authen_state_evt: unknown newstate %d", new_state);
        }
        break;
    }
}
 ...
 * @return   void
 */
void bt_ancs_app_handle_conn_mtu_info_evt(uint8_t conn_id, uint16_t mtu_size)
{
    APP_PRINT_INFO2("bt_ancs_app_handle_conn_mtu_info_evt: conn_id %d, mtu_size %d", conn_id, mtu_size);
}
 ...
 * @return   void
 */
void bt_ancs_app_handle_conn_param_update_evt(uint8_t conn_id, uint8_t status, uint16_t cause)
{
    switch (status)
    {
    case GAP_CONN_PARAM_UPDATE_STATUS_SUCCESS:
        {
            ...
            le_get_conn_param(GAP_PARAM_CONN_TIMEOUT, &conn_supervision_timeout, conn_id);
            APP_PRINT_INFO3("bt_ancs_app_handle_conn_param_update_evt update success:conn_interval 0x%x, conn_slave_latency 0x%x, conn_supervision_timeout 0x%x",
                            conn_interval, conn_slave_latency, conn_supervision_timeout);
            printf("bt_ancs_app_handle_conn_param_update_evt update success:conn_interval 0x%x, conn_slave_latency 0x%x, conn_supervision_timeout 0x%x\r\n",
                            conn_interval, conn_slave_latency, conn_supervision_timeout);
        }
        break;
    ...
    case GAP_CONN_PARAM_UPDATE_STATUS_FAIL:
        {
            APP_PRINT_ERROR1("bt_ancs_app_handle_conn_param_update_evt update failed: cause 0x%x", cause);
            printf("bt_ancs_app_handle_conn_param_update_evt update failed: cause 0x%x\r\n", cause);
        }
        break;

    case GAP_CONN_PARAM_UPDATE_STATUS_PENDING:
        {
            APP_PRINT_INFO0("bt_ancs_app_handle_conn_param_update_evt update pending.");
            printf("bt_ancs_app_handle_conn_param_update_evt update pending.\r\n");
        }
        break;
...
void bt_ancs_app_handle_gap_msg(T_IO_MSG *p_gap_msg)
{
    ...
    APP_PRINT_TRACE1("bt_ancs_app_handle_gap_msg: subtype %d", p_gap_msg->subtype);
    switch (p_gap_msg->subtype)
    {
    case GAP_MSG_LE_DEV_STATE_CHANGE:
        {
            bt_ancs_app_handle_dev_state_evt(gap_msg.msg_data.gap_dev_state_change.new_state,
    ...
    case GAP_MSG_LE_CONN_STATE_CHANGE:
        {
            bt_ancs_app_handle_conn_state_evt(gap_msg.msg_data.gap_conn_state_change.conn_id,
    ...
    case GAP_MSG_LE_CONN_MTU_INFO:
        {
            bt_ancs_app_handle_conn_mtu_info_evt(gap_msg.msg_data.gap_conn_mtu_info.conn_id,
    ...
    case GAP_MSG_LE_CONN_PARAM_UPDATE:
        {
            bt_ancs_app_handle_conn_param_update_evt(gap_msg.msg_data.gap_conn_param_update.conn_id,
    ...
    case GAP_MSG_LE_AUTHEN_STATE_CHANGE:
        {
            bt_ancs_app_handle_authen_state_evt(gap_msg.msg_data.gap_authen_state.conn_id,
...
#endif
    default:
        APP_PRINT_ERROR1("bt_ancs_app_handle_gap_msg: unknown subtype %d", p_gap_msg->subtype);
        break;
    }
}
...
T_APP_RESULT bt_ancs_app_gap_callback(uint8_t cb_type, void *p_cb_data)
{
    ...
    default:
        APP_PRINT_ERROR1("bt_ancs_app_gap_callback: unhandled cb_type 0x%x", cb_type);
        break;
    }
    return result;
}
...
T_APP_RESULT bt_ancs_app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    else  if (service_id == bt_ancs_simp_srv_id)
    {
    ...
    else if (service_id == bt_ancs_bas_srv_id)
    {
