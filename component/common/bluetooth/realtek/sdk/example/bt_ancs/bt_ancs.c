...
/** @defgroup PERIPH_ANCS_Exported_Variables ANCS Exported Variables
   ...
   * @{
   */
T_CLIENT_ID      bt_ancs_client; /**< ancs client id*/

T_APP_ANCS_LINK *bt_ancs_link_table = NULL;
uint8_t          bt_ancs_link_number;

/** End of PERIPH_ANCS_Exported_Variables
    * @}
    */
extern void *bt_ancs_evt_queue_handle;  //!< Event queue handle
extern void *bt_ancs_io_queue_handle;   //!< IO queue handle
/*============================================================================*
 *                              Functions
 *============================================================================*/
/** @defgroup PERIPH_ANCS_Exported_Functions ANCS Exported Functions
   * @{
   */
void bt_ancs_send_msg_to_app(uint8_t conn_id)
{
    T_IO_MSG io_msg;
    ...
    if (os_msg_send(bt_ancs_io_queue_handle, &io_msg, 0) == false)
    {
        GAP_PRINT_ERROR0("bt_ancs_send_msg_to_app fail1");
        printf("bt_ancs_send_msg_to_app fail1\r\n");
    }
    else if (os_msg_send(bt_ancs_evt_queue_handle, &event, 0) == false)
    {
        GAP_PRINT_ERROR0("bt_ancs_send_msg_to_app fail2");
        printf("bt_ancs_send_msg_to_app fail2\r\n");
    }
}

void bt_ancs_app_handle_notification_attribute_data(T_APP_ANCS_LINK *p_ancs_link)
{
    if (p_ancs_link->notification_attr.attribute_id != DS_NOTIFICATION_ATTR_ID_NEGATIVE_ACTION_LABEL)
    {
        ...
        if (p_ancs_link->notification_attr.attribute_id == DS_NOTIFICATION_ATTR_ID_APP_IDENTIFIER)
        {
            ...
            APP_PRINT_INFO1("parse notify attr data -- app identifiter: %s", TRACE_STRING(p_value));
            printf("parse notify attr data -- app identifiter: %s\r\n", p_value);
#endif

            if (0 == memcmp(p_value, "com.tencent.xin", 16)) //wechat
            {
            ...
            else if (0 == memcmp(p_value, "com.tencent.mqq", 16)) //TIM QQ
            {
                p_ancs_link->app_type = 4;
            }
            else if (0 == memcmp(p_value, "com.tencent.qq", 15)) //QQ
            {
        ...
        if (p_ancs_link->notification_attr.attribute_id == DS_NOTIFICATION_ATTR_ID_MESSAGE)
        {
            ...
            {
                APP_PRINT_INFO1("parse notify attr data -- MobileSMS message: %s", TRACE_STRING(p_value));
            }
            else if (p_ancs_link->app_type == 5)
            {
                APP_PRINT_INFO1("parse notify attr data -- QQ message: %s", TRACE_STRING(p_value));
            }
            else if (p_ancs_link->app_type == 1)
            {
                APP_PRINT_INFO1("parse notify attr data -- WeChat message: %s", TRACE_STRING(p_value));
                printf("parse notify attr data -- WeChat message: %s\r\n", p_value);
            }
            else if (p_ancs_link->app_type == 4)
            {
                APP_PRINT_INFO1("parse notify attr data -- MQQ message: %s", TRACE_STRING(p_value));
                printf("parse notify attr data -- MQQ message: %s\r\n", p_value);
            }
        }
        if (p_ancs_link->notification_attr.attribute_id == DS_NOTIFICATION_ATTR_ID_DATE)
        {
#if F_BT_ANCS_CLIENT_DEBUG
            APP_PRINT_INFO1("parse notify attr data -- Date: %s", TRACE_STRING(p_value));
            printf("parse notify attr data -- Date: %s\r\n", p_value);
#endif
        }
#endif
    }
    else/* All attributes has been parased*/
    {
        ...
        printf("parse notify attr: parse done\r\n");
#endif
...
void bt_ancs_app_parse_notification_attribute(T_APP_ANCS_LINK *p_ancs_link, uint8_t *p_data, uint8_t len)
{
    int i;
    ...
    for (i = 0; i < len; i++)
    {
        ...
        case DS_PARSE_ATTRIBUTE_LEN2:
            ...
            printf("parse notify attr: attribute_id %d, attribute_len %d\r\n",
                            p_ancs_link->notification_attr.attribute_id,
                            p_ancs_link->notification_attr.attribute_len);
#endif
            ...
            if (p_ancs_link->notification_attr.attribute_len > BT_ANCS_MAX_ATTR_LEN)
            {
#if F_BT_ANCS_CLIENT_DEBUG
                APP_PRINT_ERROR2("parse notify attr: error, attribute_len %d > max length %d",
                                 p_ancs_link->notification_attr.attribute_len,
                                 BT_ANCS_MAX_ATTR_LEN
                                );
#endif
                p_ancs_link->m_parse_state = DS_PARSE_NOT_START;
                memset(&p_ancs_link->notification_attr, 0, sizeof(T_DS_NOTIFICATION_ATTR));
...
#endif
                bt_ancs_app_handle_notification_attribute_data(p_ancs_link);
            }
            break;
        }
    }
...
#if F_BT_ANCS_GET_APP_ATTR
void bt_ancs_app_parse_app_attribute(T_APP_ANCS_LINK *p_ancs_link, uint8_t *p_data, uint8_t len)
{
    int i;
    for (i = 0; i < len; i++)
    {
        ...
        case DS_PARSE_APP_ATTRIBUTE_LEN2:
            ...
            printf("parse app attr: attribute_id %d, attribute_len %d\r\n",
                            p_ancs_link->app_attr.attribute_id,
                            p_ancs_link->app_attr.attribute_len);
#endif
            ...
            if (p_ancs_link->app_attr.attribute_len > BT_ANCS_MAX_ATTR_LEN)
            {
#if F_BT_ANCS_CLIENT_DEBUG
            ...
            *p_ancs_link->ptr++ = p_data[i];
            ...
            if (p_ancs_link->current_len == p_ancs_link->app_attr.attribute_len)
            {
                *p_ancs_link->ptr++ = 0;
                ...
                printf("parse app attr: command_id 0x%x, attribute_id 0x%x, attribute_len %d, data %s\r\n",
                                p_ancs_link->app_attr.command_id,
                                p_ancs_link->app_attr.attribute_id,
                                p_ancs_link->app_attr.attribute_len,
                                p_ancs_link->app_attr.data);
#endif
                p_ancs_link->m_parse_state = DS_PARSE_NOT_START;
 ...
 * @return void
 */
void bt_ancs_app_parse_data_soucre_notifications(uint8_t conn_id, uint8_t *p_data, uint8_t len)
{
    APP_PRINT_INFO1("app_parse_data_soucre_notifications: conn_id %d", conn_id);
    printf("app_parse_data_soucre_notifications: conn_id %d\r\n", conn_id);
#if F_BT_ANCS_CLIENT_DEBUG
    APP_PRINT_INFO1("data = %b", TRACE_BINARY(len, p_data));
#endif
    T_APP_ANCS_LINK *p_ancs_link = &bt_ancs_link_table[conn_id];

#if F_BT_ANCS_CLIENT_DEBUG
    APP_PRINT_INFO1("m_parse_state %d", p_ancs_link->m_parse_state);
    printf("m_parse_state %d\r\n", p_ancs_link->m_parse_state);
#endif
    ...
    {
        bt_ancs_app_parse_notification_attribute(p_ancs_link, p_data, len);
    }
#if F_BT_ANCS_GET_APP_ATTR
    else
    {
        bt_ancs_app_parse_app_attribute(p_ancs_link, p_data, len);
    }
#endif
}

void bt_ancs_app_parse_notification_source_data(uint8_t conn_id, uint8_t *p_data, uint8_t len)
{
    if (8 == len)
    {
        T_NS_DATA ns_data;
...
#if F_BT_ANCS_CLIENT_DEBUG
        APP_PRINT_INFO5("app_parse_notification_source_data: event_id %d, event_flags 0x%02x, category_id %d, "
                        "category_count %d, notification_uid 0x%08x",
                        ns_data.event_id,
                        ns_data.event_flags,
                        ns_data.category_id,
        ...
        printf("app_parse_notification_source_data: event_id %d, event_flags 0x%02x, category_id %d, "
                        "category_count %d, notification_uid 0x%08x\r\n",
                        ns_data.event_id,
                        ns_data.event_flags,
                        ns_data.category_id,
        ...
        printf("event_flags: slient %d, important %d, pre existing %d, positive action %d, negative action %d\r\n",
                        ns_data.event_flags & NS_EVENT_FLAG_SILENT,
                        ns_data.event_flags & NS_EVENT_FLAG_IMPORTANT,
                        ns_data.event_flags & NS_EVENT_FLAG_PRE_EXISTING,
                        ns_data.event_flags & NS_EVENT_FLAG_POSITIVE_ACTION,
            ...
            ns_data.category_id == NS_CATEGORY_ID_EMAIL)
        {
            ...
            if (ns_data.event_id != NS_EVENT_ID_NOTIFICATION_REMOVED)
            {
                ...
                attr_id_list[cur_index++] = LO_WORD(BT_ANCS_MAX_ATTR_LEN);
                attr_id_list[cur_index++] = HI_WORD(BT_ANCS_MAX_ATTR_LEN);
                ...
                os_msg_queue_peek(bt_ancs_link_table[conn_id].ancs_queue_handle, &msg_num);
#if F_BT_ANCS_CLIENT_DEBUG
                APP_PRINT_INFO1("app_parse_notification_source_data: msg_num %d", msg_num);
                printf("app_parse_notification_source_data: msg_num %d\r\n", msg_num);
#endif
                if (msg_num == 0)
                {
                ...
                //if queue not null or get_notification_attr write failed, then send get_notification_attr msg to queue
                ...
                if (os_msg_send(bt_ancs_link_table[conn_id].ancs_queue_handle, &ancs_msg, 0) == false)
                {
                    APP_PRINT_ERROR0("app_parse_notification_source_data: discard, msg queue is full");
                    printf("app_parse_notification_source_data: discard, msg queue is full\r\n");
                }
            }
#if F_BT_ANCS_APP_FILTER
        }
 ...
 */
T_APP_RESULT bt_ancs_client_callback(T_CLIENT_ID client_id, uint8_t conn_id, void *p_data)
{
    ...
    case ANCS_CLIENT_CB_TYPE_NOTIF_IND_RESULT:
        ...
        case ANCS_FROM_DATA_SOURCE:
            ...
            printf("ANCS_FROM_DATA_SOURCE: conn_id %d, length  %d\r\n", conn_id, 
                                    p_cb_data->cb_content.notify_data.value_size);
            bt_ancs_app_parse_data_soucre_notifications(conn_id, p_cb_data->cb_content.notify_data.p_value,
                                                p_cb_data->cb_content.notify_data.value_size);
            break;
        case ANCS_FROM_NOTIFICATION_SOURCE:
            APP_PRINT_INFO2("ANCS_FROM_NOTIFICATION_SOURCE: conn_id %d, length  %d",
                            conn_id, p_cb_data->cb_content.notify_data.value_size);
            printf("\r\nANCS_FROM_NOTIFICATION_SOURCE: conn_id %d, length  %d\r\n", conn_id, 
                                    p_cb_data->cb_content.notify_data.value_size);
            bt_ancs_app_parse_notification_source_data(conn_id, p_cb_data->cb_content.notify_data.p_value,
                                               p_cb_data->cb_content.notify_data.value_size);
            break;
        default:
            break;
    ...
    case ANCS_CLIENT_CB_TYPE_WRITE_RESULT:
        {
            ...
            case ANCS_WRITE_CONTROL_POINT:
                ...
                printf("ANCS_WRITE_CONTROL_POINT\r\n");
#if F_BT_ANCS_CLIENT_DEBUG
...
#endif
                bt_ancs_send_msg_to_app(conn_id);
                break;
    ...
    case ANCS_CLIENT_CB_TYPE_DISCONNECT_INFO:
        {
            T_ANCS_MSG ancs_msg;
            void *ancs_queue_handle = bt_ancs_link_table[conn_id].ancs_queue_handle;
            APP_PRINT_INFO1("ANCS_CLIENT_CB_TYPE_DISCONNECT_INFO: conn_id = 0x%x", conn_id);
            memset(&bt_ancs_link_table[conn_id], 0, sizeof(T_APP_ANCS_LINK));
            bt_ancs_link_table[conn_id].ancs_queue_handle = ancs_queue_handle;
            /*release msg queue*/
            while (os_msg_recv(bt_ancs_link_table[conn_id].ancs_queue_handle, &ancs_msg, 0));
        }
        break;
...
void bt_ancs_handle_msg(T_IO_MSG *p_io_msg)
{
    ...
    T_ANCS_MSG ancs_msg;
    if (os_msg_recv(bt_ancs_link_table[conn_id].ancs_queue_handle, &ancs_msg, 0) == false)
    {
        APP_PRINT_INFO1("bt_ancs_handle_msg: conn_id %d os_msg_recv failed", conn_id);
        //printf("bt_ancs_handle_msg: conn_id %d os_msg_recv failed\r\n", conn_id);
        return;
    }
    if (ancs_msg.type == ANCS_MSG_TYPE_GET_NOTIFI_ATTR)
    {
        ...
        printf("ANCS_MSG_TYPE_GET_NOTIFI_ATTR: notification_uid 0x%x\r\n",
                        ancs_msg.data.notifi_attr.notification_uid);
    ...
    else if (ancs_msg.type == ANCS_MSG_TYPE_PERFORM_NOTIFI_ACTION)
    {
        ...
        printf("ANCS_MSG_TYPE_PERFORM_NOTIFI_ACTION: notification_uid 0x%x\r\n",
                        ancs_msg.data.perform_action.notification_uid);
    ...
    else if (ancs_msg.type == ANCS_MSG_TYPE_GET_APP_ATTR)
    {
        ...
        printf("ANCS_MSG_TYPE_GET_APP_ATTR: app_identifier %s\r\n",
...
{
    uint8_t i;
    bt_ancs_link_number = link_num;
    bt_ancs_link_table = os_mem_zalloc(RAM_TYPE_DATA_ON, bt_ancs_link_number * sizeof(T_APP_ANCS_LINK));
    if (bt_ancs_link_table == NULL)
    {
        APP_PRINT_ERROR0("ancs_init: allocate buffer failed");
    }
    for (i = 0; i < bt_ancs_link_number; i++)
    {
        if (os_msg_queue_create(&(bt_ancs_link_table[i].ancs_queue_handle), BT_ANCS_MSG_QUEUE_NUM,
                                sizeof(T_ANCS_MSG)) == false)
        {
            APP_PRINT_ERROR2("ancs_init: link_num %d, i 0x%x create queue failed", link_num, i);
        }
    }
    bt_ancs_client = ancs_add_client(bt_ancs_client_callback, link_num);
}
...
void bt_ancs_deinit(void)
{
    ...
    for (i = 0; i < bt_ancs_link_number; i++)
    {
        if (bt_ancs_link_table[i].ancs_queue_handle) {
            os_msg_queue_delete(bt_ancs_link_table[i].ancs_queue_handle);
        }
        bt_ancs_link_table[i].ancs_queue_handle = NULL;
    }

    if (bt_ancs_link_table) {
        os_mem_free(bt_ancs_link_table);
        bt_ancs_link_table = NULL;
    }
}
