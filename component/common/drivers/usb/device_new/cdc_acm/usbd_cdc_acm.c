...
static u8 *cdc_acm_get_descriptor(usbd_desc_type_t desc, usbd_speed_type_t speed, u16 *len);
...
static u8 usbd_std_dev_desc[USB_LEN_DEV_DESC] USB_DMA_ALIGNED = {
...
static u8 usbd_std_lang_id_desc[USB_LEN_LANGID_STR_DESC] USB_DMA_ALIGNED = {
...
static u8 usbd_std_mfg_string_desc[CDC_ACM_MFG_STRING_DESC_SIZE] USB_DMA_ALIGNED = {
...
static u8 usbd_std_hs_product_string_desc[CDC_ACM_PRODUCT_STRING_DESC_SIZE] USB_DMA_ALIGNED = {
...
/* USB Standard Product String Descriptor for high-speed mode */
static u8 usbd_std_fs_product_string_desc[CDC_ACM_PRODUCT_STRING_DESC_SIZE] USB_DMA_ALIGNED = {
	CDC_ACM_PRODUCT_STRING_DESC_SIZE,               /* bLength */
	USB_DESC_TYPE_STRING,                           /* bDescriptorType */
	'R',                                            /* bString */
	'e',
...
static u8 usbd_std_sn_string_desc[CDC_ACM_SN_STRING_DESC_SIZE] USB_DMA_ALIGNED = {
...
static u8 usbd_std_device_qualifier_desc[USB_LEN_DEV_QUALIFIER_DESC] USB_DMA_ALIGNED = {
...
static u8 usbd_cdc_hs_config_desc[CDC_ACM_CONFIG_DESC_SIZE] USB_DMA_ALIGNED = {
	...
	USB_HIGH_BYTE(CDC_ACM_HS_BULK_MAX_PACKET_SIZE),
	0x00                                            /* bInterval */
};  /* usbd_cdc_hs_config_desc */

/* USB CDC ACM Device Full Speed Configuration Descriptor */
static u8 usbd_cdc_fs_config_desc[CDC_ACM_CONFIG_DESC_SIZE] USB_DMA_ALIGNED = {
	/* USB Standard Configuration Descriptor */
	USB_LEN_CFG_DESC,                               /* bLength */
	USB_DESC_TYPE_CONFIGURATION,                    /* bDescriptorType */
	CDC_ACM_CONFIG_DESC_SIZE,                       /* wTotalLength */
	...
	USB_HIGH_BYTE(CDC_ACM_FS_BULK_MAX_PACKET_SIZE),
	0x00                                            /* bInterval */
};  /* usbd_cdc_fs_config_desc */

/* USB CDC ACM Device Other Speed Configuration Descriptor */
static u8 usbd_cdc_other_speed_config_desc[CDC_ACM_CONFIG_DESC_SIZE] USB_DMA_ALIGNED = {
	/* USB Standard Configuration Descriptor */
	USB_LEN_CFG_DESC,                               /* bLength */
	...
	USB_HIGH_BYTE(CDC_ACM_FS_BULK_MAX_PACKET_SIZE),
	0x00                                            /* bInterval */
};  /* usbd_cdc_other_speed_config_desc */
...
static u8 cdc_acm_set_config(usb_dev_t *dev, u8 config)
{
	...
	usbd_ep_init(dev, CDC_ACM_BULK_IN_EP, USBD_EP_TYPE_BULK, ep_mps);

	/* Init BULK OUT EP */
	ep_mps = (dev->dev_speed == USBD_SPEED_HIGH) ? CDC_ACM_HS_BULK_OUT_PACKET_SIZE : CDC_ACM_FS_BULK_OUT_PACKET_SIZE;
	usbd_ep_init(dev, CDC_ACM_BULK_OUT_EP, USBD_EP_TYPE_BULK, ep_mps);

	/* Init INTR IN EP */
	usbd_ep_init(dev, CDC_ACM_INTR_IN_EP, USBD_EP_TYPE_INTR, CDC_ACM_INTR_IN_PACKET_SIZE);
...
static u8 *cdc_acm_get_descriptor(usbd_desc_type_t desc, usbd_speed_type_t speed, u16 *len)
{
	...
	case USBD_DESC_CONFIG:
		if (speed == USBD_SPEED_HIGH) {
			buf = usbd_cdc_hs_config_desc;
			*len = sizeof(usbd_cdc_hs_config_desc);
		} else {
			buf = usbd_cdc_fs_config_desc;
			*len = sizeof(usbd_cdc_fs_config_desc);
		}
		break;
	case USBD_DESC_OTHER_SPEED_CONFIG:
		/* speed == USBD_SPEED_HIGH */
		buf = usbd_cdc_other_speed_config_desc;
		*len = sizeof(usbd_cdc_other_speed_config_desc);
		break;
...
u8 usbd_cdc_acm_transmit(u8       *buf, u16 len)
{
	...
	if (cdev->bulk_in_state == 0U) {
		...
		//dev->ep_in[CDC_ACM_BULK_IN_EP & 0xFU].data_len = len; // TODO: move it to usbd_ep_transmit
