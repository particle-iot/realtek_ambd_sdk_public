...
#define MOUNT_RETRY_TIMES		5

static u32 filenum = 0;
_sema MscSema;

static void msc_attach(void)
{
	rtw_up_sema(&MscSema);
}

struct us_usr_cb msc_usr_cb = {
	.attach = msc_attach,
	//.detach = msc_detach,
};
...
void example_usbh_msc_thread(void *param)
{
	...
	rtw_init_sema(&MscSema, 0);
	usb_msc_register_usr_cb(&msc_usr_cb);
